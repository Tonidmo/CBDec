...
namespace CBlib
{
   ...
   {
      m_a_u1_checks.reserve(MAX_CHECKS_BUFF_LEN);
      m_a_u1_events.reserve(MAX_EVENTS_BUFF_LEN);

      memset(&m_a_u1_checks, 0, MAX_CHECKS_BUFF_LEN);
      memset(&m_a_u1_events, 0, MAX_EVENTS_BUFF_LEN);
   }

   ClosedBranch::ClosedBranch(std::vector<bool> const & a_u1_checks, std::vector<bool> const & a_u1_events)
   {
      if (MAX_CHECKS_BUFF_LEN >= a_u1_checks.size())
      {
         m_a_u1_checks = a_u1_checks;
      }
      else
      {
         std::copy_n(a_u1_checks.begin(), MAX_CHECKS_BUFF_LEN, m_a_u1_checks.begin());
         CBLIB_WARN("Possible information loss! Increase max buffer size...");
      }

      if (MAX_EVENTS_BUFF_LEN >= a_u1_events.size())
      {
         m_a_u1_events = a_u1_events;
      }
      else
      {
         std::copy_n(a_u1_events.begin(), MAX_CHECKS_BUFF_LEN, m_a_u1_events.begin());
         CBLIB_WARN("Possible information loss! Increase max buffer size...");
      }
   }
   ...
   ECBStatus ClosedBranch::add_check_to_closed_branch(uint64_t const u64_new_check)
   {
      ...
      if (u64_new_check > MAX_CHECKS_BUFF_LEN)
      {
   ...
   ECBStatus ClosedBranch::add_event_to_closed_branch(uint64_t const u64_new_event)
   {
      ...
      if (u64_new_event > MAX_EVENTS_BUFF_LEN)
      {
         CBLIB_ERROR("Error (%d)! Increment the maximum buffer size...", E_CB_ERR_INVAL);
         e_ret = E_CB_ERR_INVAL;
      }
